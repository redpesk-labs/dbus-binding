...
static struct sd_bus *getbus(int system)
{
	static struct sd_bus *buses[2] = { NULL, NULL };
	int index = !!system;
	if (buses[index] == NULL)
		(index ? sd_bus_default_system : sd_bus_default_user)(&buses[index]);
	return buses[index];
}
...
static void rawcall(afb_req_t req)
{
	...
	if (busname == NULL || !strcmp(busname, "user"))
		bus = getbus(0);
	else if (!strcmp(busname, "system"))
		bus = getbus(1);
	else
		goto bad_request;
	if (bus == NULL)
